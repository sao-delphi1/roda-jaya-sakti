unit ARRptKartuPiutangPRDetilDlg;
...
type
  TfmARRptKartuPiutangPRDetilDlg = class(TfmRptDlg)
    GroupBox1: TGroupBox;
    dt1: TdxDateEdit;
    rbAll: TRadioButton;
...
var
  fmARRptKartuPiutangPRDetilDlg: TfmARRptKartuPiutangPRDetilDlg;

implementation

uses StdLv0, QRRptAPKartuHutang, UnitGeneral, QRRptAPKartuHutangDetil,
  ARQRRptKartuPiutangDetil, ARQRRptKartuPiutangPRDetil, Search;

{$R *.dfm}

procedure TfmARRptKartuPiutangPRDetilDlg.rbAllClick(Sender: TObject);
begin
  inherited;
...
procedure TfmARRptKartuPiutangPRDetilDlg.FormShow(Sender: TObject);
begin
  inherited;
  quAct.Open;        
  dt1.Date:=date;
...
procedure TfmARRptKartuPiutangPRDetilDlg.bbPreviewClick(Sender: TObject);
begin
  inherited;
  with TfmARQRRptKartuPiutangPRDetil.Create(Self) do
  try
    tglDari:=dt1.Date;
    ...
    Begin
       ...
       add(' Select Distinct A.CustId, A.CustName,(A.CustId+''-''+A.CustName) as Cust from ARMsCustomer A '
          +' INNER JOIN ARTrPenggantiReturPenjHd B ON A.CustId=B.CustId '
         ...
         Add(' -isnull((Select isnull(Sum(ValuePayment),0) FROM ARTrPayReturnRepDt N INNER JOIN ARTrPayReturnRepHd M ON N.PayReturnRepId=M.PayReturnRepId'
            +' WHERE N.ReturnRepId=B.ReturnrepID AND CONVERT(varchar(8),M.TransDate,112) <= '''+FormatDateTime('yyyyMMdd',dt1.Date)+'''),0)');
          Add(' <> 0 ');
    ...
    Begin
       ...
       add(' Select Distinct B.CustId,A.CurrID, A.CurrName,(A.CurrId+''-''+A.CurrName) as Valuta from SAMsValuta A '
          +' INNER JOIN ARTrPenggantiReturPenjHd B ON A.CurrId=B.CurrId'
...
procedure TfmARRptKartuPiutangPRDetilDlg.bbCancelClick(Sender: TObject);
begin
  inherited;
