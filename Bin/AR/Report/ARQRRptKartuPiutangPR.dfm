inherited fmARQRRptKartuPiutangPR: TfmARQRRptKartuPiutangPR
  Left = 24
  Top = 66
  Width = 1000
  HorzScrollBar.Position = 85
  Caption = 'fmARQRRptKartuPiutangPR'
  PixelsPerInch = 96
  TextHeight = 13
  ...
  inherited qu003: TADOQuery
    SQL.Strings = (
        ...
        ','
      'K.ReturnRepID as IP, isnull(K.TTLReturRep,0) as Kredit,'
      
        ' (select isnull(sum(L.ValuePayment),0) FROM ARTrPayReturnRepDt L' +
        ' '
      'where L.ReturnRepID = K.ReturnRepID) as Debit FROM '
      'ARTrPenggantiReturPenjHd K')
    Left = 316
    Top = 100
  end
end
