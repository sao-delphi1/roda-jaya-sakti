...
type
  TfmARTrReturPenjualan = class(TfmStdLv41)
    ...
    quLoot: TADOQuery;
    dsLoot: TDataSource;
    dbgBatchColumn7: TdxDBGridColumn;
    dbgBatchColumn8: TdxDBGridColumn;
    quLootSNID: TStringField;
    quLootReturnID: TStringField;
    quLootSaleId: TStringField;
    quLootItemID: TStringField;
    quLootPrice: TBCDField;
    quLootWareHouseId: TStringField;
    quLootUpdDate: TDateTimeField;
    quLootUpdUser: TStringField;
    quLootModal: TBCDField;
    quLootPurchaseID: TStringField;
    quLootFgSN: TStringField;
    quLootQty: TBCDField;
    quLootExpDate: TDateTimeField;
    quLootNumAll: TAutoIncField;
    ...
    procedure dxDBGrid2SNIDButtonClick(Sender: TObject;
      AbsoluteIndex: Integer);
    procedure quLootAfterPost(DataSet: TDataSet);
    procedure quLootBeforeDelete(DataSet: TDataSet);
...
uses StdLv2, StdLv1, StdLv0, Search, UnitGeneral, ConMain, StdLv3, MyUnit,
  Allitem, ARQRRptSuratJalan, ARTrPengReturPenj,QRRetur, RptLv0;

{$R *.dfm}

procedure TfmARTrReturPenjualan.CekTotal;
...
Begin
   ...
   begin
      ...
      Add('SELECT ISNULL(SUM(CASE WHEN K.FgTax=''Y'' THEN ((K.Price*K.Qty)-((K.Price*K.Qty)*K.Disc*0.01))*1.1 ELSE (K.Price*K.Qty)-((K.Price*K.Qty)*K.Disc*0.01) END),0) as TTLRetur, '
         ...
         +'SELECT A.ReturnId,A.SaleID,A.Price,A.Qty,(SELECT B.FgTax FROM ARTrKonInvPelHD B Where B.KonInvPelId=A.SaleId) as FgTax, '
         +'(SELECT B.Discount FROM ARTrKonInvPelHD B Where B.KonInvPelId=A.SaleID) as Disc '
         +'FROM ARTrReturPenjualanDT A WHERE A.ReturnID='''+quMainReturnID.AsString+''' ) as K ');
      Open;
   end;
   ...
   begin
      Close; Clear;
      Add('UPDATE ARTrReturPenjualanHD SET TTLRetur='''+CurrToStr(Total)+''',TTLPPN='''+CurrToStr(PPN)+''',TTLDiskon='''+CurrToStr(Disc)+''' WHERE ReturnID='''+quMainReturnID.AsString+''' ');
      ExecSQL;
   end;
   quTotal.Requery();
End;
...
begin
  ...
  with TfmSearch.Create(Self) do
    try
       Title := 'Cari Barang';
       SQLString := ' SELECT Distinct A.Itemname as Nama_Barang,A.ItemId as Kode_Barang,'
                   +' ProductDesc as Product, GroupDesc as Group_Barang '
                   +' FROM ARTRKonInvPelDT A INNER JOIN ARTRKonInvPelHD E ON A.KonInvPelID=E.KonInvPelID'
                   ...
                   +' WHERE E.CustId='''+quMainCustID.Value+''''
                   +' ORDER BY A.ItemID';
       if ShowModal = MrOK then
       begin
...
begin
  ...
  quMainRekeningDisc.AsString := ARDisc;
  quMainRekeningD.AsString := sDRPj; //piutang
  quMainRekeningK.AsString := sDGRPj; //persediaan
...
begin
  ...
  Begin
   ...
   Begin
     Close;Clear;
     add(' Select Top 1 B.VoucherID PiutangID FROM CFTrKKBBdt B'
        +' WHERE  B.Note='''+quDetilSaleId.AsString +'''');
     Open;
     if Not IsEmpty then
     Begin
...
begin
  ...
  DeleteFromAllItem(quMainReturnID.AsString,quDetilSaleId.AsString,qudetilitemId.AsString,
                    quDetilWareHouseID.AsString,abs(quDetilPrice.AsFloat),8,1,'');
...
begin
  ...
  with TfmSearch.Create(Self) do
    try
       ...
       SQLString := ' Select Distinct A.konInvPelId as Nota_Penjualan,'
                   +' Convert(varchar(10),Transdate,103) as Tgl_Penjualan,Price as Harga_Jual_Barang,'
                   +' isnull(Qty,0)-(Select isnull(Sum(Qty),0) FROM ARTrReturPenjualanDt D WHERE D.SaleID=A.konInvPelId '
                   +' AND Itemid='''+quDetilItemID.Value+''' AND D.Price=B.Price) as Sisa'
                   +' FROM ARTrKonInvPelHD A INNER JOIN ARTrKonInvPelDt B ON A.konInvPelId=B.konInvPelId'
                   ...
                   +' WHERE D.SaleID=A.konInvPelId AND Itemid='''+quDetilItemID.Value+''' AND D.Price=B.Price) <> 0 '
                   +' ORDER BY A.konInvPelId ' ;
       if ShowModal = MrOK then
       begin
          if quDetil.State = dsBrowse then quDetil.Edit;
          quDetilSaleID.Value := Res[0];
          quDetilPrice.AsString:= Res[2];
...
begin
  ...
  DeleteFromAllItem(quMainReturnID.Value,quDetilSaleId.Value,qudetilitemId.Value,
                    quDetilWareHouseID.Value,quDetilPrice.value,8,1,'');
end;
...
begin
  ...
  DeleteFromAllItem(quMainReturnID.Value,8,1,'');
end;
...
begin
  inherited;
   with TfmQRRetur.Create(Self) do
     try
       qrlTitle.Caption := 'RETUR PENJUALAN';
       qrlPeriode.Caption :='Tanggal : '+FormatDateTime('dd/mm/yyyy',quMainTransDate.AsDateTime);
       QRLabel1.Caption := quMainReturnID.AsString;

       with qu001,SQL do
       Begin
         Close;Clear;
         add(' Select ReturnId,A.CustId,C.CustName,C.Address+'' ''+C.City as Address '
            +' FROM ARTrReturPenjualanHd A INNER JOIN ARMsCustomer C ON A.CustId=C.CustId'
            +' WHERE A.ReturnId='''+quMainReturnID.AsString+'''');
         Open;
       End;
       ...
       Begin
         Close;Clear;
         add(' Select A.ItemId,B.ItemName,A.SaleId,B.UOMId,ISNULL(Sum(A.Qty),0) as Qty,B.UOMID2 FROM ARTrReturPenjualanDt A'
            +' INNER JOIN INMsItem B ON A.Itemid=B.Itemid'
            +' INNER JOIN INMsProduct C ON B.ProductId=C.ProductId WHERE A.ReturnId=:ReturnId'
            +' GROUP BY A.ItemId,B.ItemName,B.UOMId2,B.UOMID,A.SaleID');
...
procedure TfmARTrReturPenjualan.quLootAfterPost(DataSet: TDataSet);
begin
  inherited;
  with quAct,SQL do
  begin
   ...
   Add('delete from AllLootNumber where '
      +'fgnum='''+quLootNumAll.AsString+''' and voucherno='''+quMainReturnID.AsString+''' and snid='''+quLootSNID.AsString+''' and warehouseid='''+quDetilWareHouseID.AsString+''' ');
   ExecSQL;
  End;
  with quAct,SQL do
  begin
   ...
   Add('insert AllLootNumber (fgNum,voucherno,TransDate,FgTrans,SNID,qty,expdate,warehouseid,itemid) '
      +'select '''+quLootNumAll.AsString+''','''+quMainReturnID.AsString+''','''+FormatDatetime('yyyy-mm-dd',quMainTransDate.AsDatetime)+''',5,'
      +''''+qulootSNID.AsString+''','''+qulootQty.AsString+''','''+FormatDatetime('yyyy-mm-dd',quLootExpDate.AsDatetime)+''','''+quDetilWareHouseID.AsString+''','''+quDetilItemID.AsString+''' ');
   ExecSQL;
  End;
end;

procedure TfmARTrReturPenjualan.quLootBeforeDelete(DataSet: TDataSet);
begin
  inherited;
  with quAct,SQL do
  begin
...
procedure TfmARTrReturPenjualan.dbgBatchSNIDButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
  try
    ...
    if ShowModal = MrOK then
    begin
       if quLoot.State = dsBrowse then quLoot.Edit;
       quLootSNID.AsString     := Res[0];
       quLootExpDate.AsDatetime  := StrtoDatetime(Res[2]);
    end;
  finally
    free;
  end;
...
procedure TfmARTrReturPenjualan.quDetilAfterDelete(DataSet: TDataSet);
begin
  inherited;
  CekTotal;
end;

procedure TfmARTrReturPenjualan.quMainAfterPost(DataSet: TDataSet);
begin
  inherited;
  CekTotal;
end;

procedure TfmARTrReturPenjualan.dxButton4Click(Sender: TObject);
begin
  inherited;
  quLoot.Cancel;
end;

procedure TfmARTrReturPenjualan.dxButton5Click(Sender: TObject);
begin
  inherited;
  if Self.quLoot.State = dsInsert then
  begin
   Self.quLoot.Edit;
   Self.quLoot.Post;
   Self.quLoot.Requery();
  end;
  if Self.quLoot.State = dsEdit then
  begin
   quLoot.Post;
  end;
end;

procedure TfmARTrReturPenjualan.dxButton2Click(Sender: TObject);
begin
  inherited;
  quLoot.Append;
end;

procedure TfmARTrReturPenjualan.dxButton1Click(Sender: TObject);
begin
  inherited;
  if quLoot.IsEmpty then Abort;
  if (dsLoot <> nil) and (dsLoot.DataSet <> nil) and
     (MessageDlg('Hapus Data ?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
        dsLoot.DataSet.Delete;
end;
