...
type
  ...
  public
    ...
    rek,nmrek,cabrek,bankrek,telpcomp,AutoInc,FgOto : string;
  end;
...
procedure TfmARTrKonInvPelIDR.CekUserEdit;
Begin
  if FgEdit<>'Y' then
  begin
...
procedure TfmARTrKonInvPelIDR.CekUserDelete;
Begin
  if FgDelete<>'Y' then
  begin
    if (quMainUpdUser.ASString<>dmmain.userid) then
...
procedure TfmARTrKonInvPelIDR.CekUserTambah;
Begin
  if FgTambah<>'Y' then
  begin
    if (quMainUpdUser.ASString<>dmmain.userid) then
...
Begin
  ...
  try
   ...
   { end else
    ...
    QRImage1.Enabled := True;
    QRImage8.Enabled := False;
    QRLabel35.Caption := 'PT TIMUR PERKASA MINERALINDO';
    QRLabel36.Caption := 'Komp. Kokan Plaza Kelapa Gading Blok A 21 & 23, Kelapa Gading Barat, '+ char(13) +
    ...
    Begin
      Close;Clear;
      add('SELECT A.KonInvPelID as SaleId,C.CustName,C.Address,C.CPerson,A.CURRID,A.Note,''Faktur : ''+A.FPS as FPS,A.PeriodeCharge,ISNULL(A.NoKontrak,'''') as NoKontrak,A.SaleID as Kontrak '
...
Begin
  ...
  try
    ...
    Begin
      Close;Clear;
      add('SELECT A.KonInvPelID as SaleId,C.CustName,C.Address,C.CPerson,A.CURRID,A.Note,A.PeriodeCharge,ISNULL(A.NoKontrak,'''') as NoKontrak,A.SaleID as Kontrak, '
...
begin
 ...
 { if qumain.State = dsInsert then
  ...
  If TRIM(quMainSaleID.AsString)='' then
  Begin
    MsgInfo('No Kontrak tidak boleh kosong');
 ...
 { with quAct,SQL do
  begin
    close;clear;
    add('Select SaleID from ARTrSalesKontrakHd where SaleID='''+quMainSaleID.AsString+''' ');
    open;
    if IsEmpty then
    begin
      ShowMessage('Data Sales kontrak tidak ada di transaksi Sales Kontrak');
      abort;
    end;
  end;     }
...
begin
  ...
  //SetReadOnly(dxDBGrid1UserPrice,TRUE);
...
begin
  ...
  begin
    Close;Clear;
    if StForm = 'RJS' then
    Add('Select ISNULL(RekJual,''R01'') as RekeningJual FROM SAMsSet ')
    else
    Add('Select ISNULL(RekJualTIM,''R01'') as RekeningJual FROM SAMsSet ');
    Open;
  end;

  quMainRekJual.AsString := quAct.FieldByName('RekeningJual').AsString;
...
begin
  ...
  { with quAct, SQL do
   begin
      Close; Clear;
      Add('select ISNULL(A.Qty,0)-ISNULL((SELECT SUM(B.Qty) FROM ARTrKonInvPelDt B inner join ARTrKonInvPelHd X on B.KonInvPelID=X.KonInvPelID '
         +'WHERE X.SaleID=A.SaleID AND A.NoUnit=B.NoUnit AND B.KonInvPelID<>'''+quMainKonInvPelID.ASString+'''),0) as Jum from ARTrSalesKontrakDt A '
         +'Where SaleID='''+quMainSaleID.ASString+''' AND A.NoUnit='''+quDetilNoUnit.ASString+''' ');
      Open;
   end;

   JumPOSbm := quAct.FieldByName('Jum').AsCurrency;
   if JumPOSbm-quDetilQty.AsCurrency<0 then
   begin
     ShowMessage('Unit di sales kontrak tidak mencukupi, Sisa Unit di SK : '+Currtostr(JumPOSbm)+'.');
     Abort;
   end;      }
...
begin
  ...
  if quMainFgOto.AsString = 'T' then
    quMainLFgOto.AsString := 'Pending'
  else
    quMainLFgOto.AsString := 'Approved';
end;
...
begin
  ...
  begin
    Close;Clear;
    Add('INSERT ARTrKonInvPelDt (KonInvPelID,KonTransBrgID,NoUnit,FgPPH23,ItemID,Price,Qty,UpdUser,UpdDate,UOMID,ItemName) ');
    Add('SELECT '''+quMainKonInvPelID.AsString+''','''+quMainSOID.AsString+''',A.NoUnit,A.FgPPH23,A.ItemID,A.Price, '
       +'A.Qty,'
       +' '''+dmMain.UserId+''',A.UpdDate,A.UOMID,A.ItemName '
       +'FROM ARTrSalesKontrakDt A INNER JOIN ARTrSalesKontrakHd B ON A.SaleID=B.SaleID '
       +'WHERE B.SaleID='''+quMainSaleID.AsString+''' AND A.ItemID NOT IN (SELECT X.ItemID FROM ARTrKonInvPelDt X '
       +'WHERE X.KonInvPelID='''+quMainKonInvPelID.AsString+''' AND B.SaleID='''+quMainSaleID.AsString+''') '
       //+'AND A.Qty-ISNULL((select SUM(X.Qty) from ARTrKonInvPelDt X inner join ARTrKonInvPelHd Y on X.KonInvPelID=Y.KonInvPelID '
       //+'WHERE Y.SaleID=B.SaleID AND X.NoUnit=A.NoUnit),0) <> 0 '
       +'ORDER BY A.UpdDate');
    ExecSQL;
  end;
...
begin
  ...
  begin
    Close;Clear;
    add('Select KonInvPelID,FgOto from ARTrKonInvPelHd where KonInvPelID='''+quMainKonInvPelID.AsString+''' ');
    open;
  end;
  FgOto := quAct.FieldByName('FgOto').AsString;

  if FgOto = 'T' then
  begin
  ShowMessage('Invoice ini belum di otorisasi, mohon otorisasi agar bisa cetak');
  abort;
  end else
...
begin
  ...
  try
    Title := 'Pilih Nomor Unit';
     SQLString := 'select K.NoUnit,K.ItemName,K.Qty as Jumlah,K.Price as Harga,K.UOMID as Satuan,K.JumTerima as Terima,K.ItemName as Keterangan FROM (  '
                +'select A.SaleID,A.NoUnit,B.Description,A.Jumlah,A.Price,A.UOMID,A.ItemName,A.ItemID,A.Qty, '
                +'ISNULL((select SUM(X.Qty) from ARTrKonInvPelDt X inner join ARTrKonInvPelHd Y on X.KonInvPelID=Y.KonInvPelID '
                +'WHERE Y.SaleID=A.SaleID AND X.NoUnit=A.NoUnit),0) as JumTerima '
                +'from ARTrSalesKontrakDt A '
                +'inner join INMsNoUnit B on A.NoUnit=B.NoUnit ) as K '
                +'where K.SaleID='''+quMainSaleID.AsString+''' AND K.NoUnit NOT IN (Select NoUnit from ARTrKonInvPelDt Where KonInvPelID='''+quMainKonInvPelID.AsString+''') ';
    {SQLString := 'select K.NoUnit,K.ItemName,K.Qty-K.JumTerima as Jumlah,K.Price as Harga,K.UOMID as Satuan,K.JumTerima as Terima,K.ItemName as Keterangan FROM (  '
                +'select A.SaleID,A.NoUnit,B.Description,A.Jumlah,A.Price,A.UOMID,A.ItemName,A.ItemID,A.Qty, '
                +'ISNULL((select SUM(X.Qty) from ARTrKonInvPelDt X inner join ARTrKonInvPelHd Y on X.KonInvPelID=Y.KonInvPelID '
                +'WHERE Y.SaleID=A.SaleID AND X.NoUnit=A.NoUnit),0) as JumTerima '
                +'from ARTrSalesKontrakDt A '
                +'inner join INMsNoUnit B on A.NoUnit=B.NoUnit ) as K '
                +'where K.SaleID='''+quMainSaleID.AsString+''' AND ISNULL(K.Qty-K.JumTerima,0)<>0';      }
    if ShowModal = MrOK then
    begin
       if qudetil.State = dsBrowse then qudetil.Edit;
       ...
       quDetilUOMID.Value := Res[4];
       quDetilItemName.Value := Res[6];
    end;
  finally
    free;
  end;
...
procedure TfmARTrKonInvPelIDR.dxDBButtonEdit17ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
    try
       Title := 'Transaksi Sales Kontrak';
       SQLString := 'SELECT DISTINCT K.SaleID, K.CustID,L.CustName,CONVERT(VARCHAR(10),K.TransDate,103) as Tanggal,K.SiteID,K.JatuhTempo,K.CurrID,K.FgTax as Pajak,K.Rate FROM( '
                   +'SELECT A.SaleID,A.TransDate,A.SiteID,A.JatuhTempo,A.CustID,A.CurrID,A.FgTax,A.BankID,A.Rate,ISNULL ((SELECT SUM(X.Qty) FROM ARTrKonInvPelDt X '
                   +'INNER JOIN ARTrKonInvPelHd Y ON X.KonInvPelID=Y.KonInvPelID WHERE Y.SaleID=A.SaleID AND '
                   +'Y.CustID=A.CustID),0) as JumTerima '
                   +'FROM ARTrSalesKontrakHd A '
                   +') as K '
                   +'INNER JOIN ARMsCustomer L ON K.CustID=L.CustID '
                   //+'WHERE ISNULL (K.Qty-K.JumTerima,0) > 0 '
                   +'WHERE CONVERT(VARCHAR(8),K.Transdate,112) <='''+FormatDateTime('yyyyMMdd',quMainTransDate.AsDateTime)+''' '
                   +'AND K.BankID='''+StForm+''' '
                   +'ORDER BY K.SaleID ';
      { SQLString :=  'Select PR.GRNID as Nomer_Penermiaan , S.SuppName as Supplier , TransDate as Tanggal , PR.Note as Keterangan from '
                     +'APTrPurchaseReceiveHD PR inner join APmsSupplier S on PR.SuppID = S.SuppID';     }
       if ShowModal = MrOK then
       begin
         quMainSaleID.Value := Res[0];
         quMainCustID.Value := Res[1];
         quMainSiteID.Value := Res[4];
         quMainJatuhTempo.AsString := Res[5];
         quMainCurrID.value := Res[6];
         quMainFgTax.value := Res[7];
         quMainRate.AsCurrency := StrToCurr(Res[8]);
       end;
    finally
       free;
    end;
