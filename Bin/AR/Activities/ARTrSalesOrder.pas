...
type
  TfmARTrSalesOrder = class(TfmStdLv31)
...
var
  fmARTrSalesOrder: TfmARTrSalesOrder;

implementation

uses ConMain, Search, UnitGeneral, MyUnit,
  ARQRRptSuratJalan, Allitem, ARSN,RptMultiCetak;

{$R *.dfm}

procedure TfmARTrSalesOrder.CeKStatus;
Begin
  with quAct, SQL do
  begin
    Close; Clear;
...
procedure TfmARTrSalesOrder.CekStatusDetil;
Begin
  with quAct, SQL do
     begin
        Close; Clear;
        Add('SELECT TOP 1 KonInvPelID FROM ARTrKonInvPelDt '
           +' WHERE KonTransBrgID='''+quMainPOID.AsString+''' AND ItemID='''+quDetilItemID.AsString+''' ');
        Open;
        if not IsEmpty then
        begin
           MsgInfo('Sudah ada Invoice ['+quAct.FieldByName('KonInvPelID').AsString+'], tidak bisa edit/Delete lagi');
...
procedure TfmARTrSalesOrder.CeKretur;
Begin
  with quAct, SQL do
     begin
        Close; Clear;
        Add(' SELECT top 1 KonReturID FROM ARTrKonReturDt  '
           +' WHERE KonTransBrgID='''+quMainPOID.Value+'''');
        Open;
        if not IsEmpty then
        begin
...
procedure TfmARTrSalesOrder.FormShow(Sender: TObject);
begin
...
procedure TfmARTrSalesOrder.quMainCalcFields(DataSet: TDataSet);
begin
  inherited;
with quAct,SQL do
  begin
...
procedure TfmARTrSalesOrder.dsMainStateChange(Sender: TObject);
begin
  inherited;
...
procedure TfmARTrSalesOrder.quMainNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmARTrSalesOrder.quMainAfterPost(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmARTrSalesOrder.quMainBeforeDelete(DataSet: TDataSet);
begin
  inherited;
  CeKStatus;
  CeKretur;
...
procedure TfmARTrSalesOrder.quMainBeforeEdit(DataSet: TDataSet);
begin
  inherited;
  if quMain.State <> dsInsert then
...
procedure TfmARTrSalesOrder.quMainBeforePost(DataSet: TDataSet);
var ST: String;
begin
  inherited;
...
procedure TfmARTrSalesOrder.dxDBButtonEdit1ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
...
procedure TfmARTrSalesOrder.dxDBButtonEdit4ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
