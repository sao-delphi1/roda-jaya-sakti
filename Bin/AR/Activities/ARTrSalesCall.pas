...
uses
  ...
  dxDBELib, dxEditor, DBCtrls, ExtDlgs;
...
type
  TfmARTrSalesCall = class(TfmStdLv31)
    ...
    dxButton1: TdxButton;
    quMainNamaBarang: TStringField;
    quMainSpesifikasi: TStringField;
    quMainHarga: TBCDField;
    Label8: TLabel;
    Label9: TLabel;
    Label10: TLabel;
...
uses MyUnit, Search, UnitGeneral, ConMain, StdLv1, ARQRRptPerkenalan,
  UnitDate;
...
begin
  ...
  SetReadOnly(dxDBEdit6,quMain.State<>dsInsert);
  SetReadOnly(dxDBEdit7,quMain.State<>dsInsert);
  SetReadOnly(dxDBEdit8,quMain.State<>dsInsert);
  SetReadOnly(dxDBEdit10,quMain.State<>dsInsert);
...
begin
  ...
  quMainEndDate.AsDateTime := EncodeDate(GetYear(Date),GetMonth(Date),30);
  quMainCurrID.AsString := 'IDR';
  quMainUP.AsString := '';
  quMainHarga.AsCurrency := 0;
end;
...
begin
  ...
  with TfmSearch.Create(Self) do
    try
       ...
       SQLString := 'SELECT A.SCID as No,CONVERT(VARCHAR(10),A.Transdate,103) as Tgl,A.Respon,'
                   +'A.Note,A.CurrID,A.Customer,A.UP,A.Phone,A.Fax,A.Email,'
...
procedure TfmARTrSalesCall.dxButton1Click(Sender: TObject);
begin
  inherited;
  ...
  with TfmRptPerkenalan.Create(Self) do
  try
    with qu001,SQL do
    Begin
      Close;Clear;
      add('SELECT CONVERT(VARCHAR(10),A.Transdate,103) as Tanggal,A.Customer,A.UP,A.Phone,A.Fax,A.SalesID+''/''+A.SCID as Reff,'
         +'A.SCID as GBUID,B.SalesName,A.Email,ISNULL(A.Harga,0) as Harga,A.NamaBarang,A.Spesifikasi,A.Foto '
         +'FROM ARTrSCHd A INNER JOIN ARMsSales B ON A.SalesID=B.SalesID '
         +'WHERE A.SCID='''+quMainSCID.AsString+'''');
      Open;
    End;
    qlbPromo.Caption := '4. Penawaran Harga berlaku sampai dengan tanggal '+ FormatDateTime('dd/MM/yyyy',quMainEndDate.AsDateTime);
     if sCetak = '0' then
...
procedure TfmARTrSalesCall.bbLogoClick(Sender: TObject);
begin
  inherited;
...
procedure TfmARTrSalesCall.bbHpsLogoClick(Sender: TObject);
begin
  inherited;
