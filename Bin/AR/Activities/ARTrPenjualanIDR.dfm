inherited fmARTrPenjualanIDR: TfmARTrPenjualanIDR
  ...
  inherited pcMain: TdxPageControl
    ...
    inherited ts02: TdxTabSheet
      ...
      object dbgPenjualan: TdxDBGrid
        ...
        object dbgPenjualanQty: TdxDBGridMaskColumn
          Caption = 'Jumlah'
          Width = 56
          BandIndex = 0
          RowIndex = 0
      ...
      object dbgSerialNumber: TdxDBGrid
        ...
        object dbgSerialNumberSaleID: TdxDBGridButtonColumn
          Visible = False
          Width = 99
          BandIndex = 0
          RowIndex = 0
        ...
        object dbgSerialNumberItemId: TdxDBGridMaskColumn
          Visible = False
          Width = 99
          BandIndex = 0
          RowIndex = 0
      ...
      object dxDBFgTrans: TdxDBImageEdit
        Left = 301
        Top = 25
        Width = 121
        TabOrder = 18
        OnKeyPress = dxDBEdit1KeyPress
        DataField = 'FlagCounter'
        DataSource = dsMain
        StyleController = SCEdit
  ...
  object quTotal: TADOQuery
    ...
    SQL.Strings = (
      'Select round(SUM (Qty*(Price+Komisi)),0) as SubTotal,'
      '(isnull(A.Charge,0)/100)*(SUM (Qty*(Price+Komisi)))  as Charge,'
      'round(A.Discount,0)  , A.Administrasi  ,'
      
        'round(Case When A.FgTax= '#39'Y'#39' then ROUND((sum(qty*(Price+Komisi))' +
        '-A.Discount)*10/100,0) else 0 end,0) as Pajak,'
      
        'round(SUM (Qty*(Price+Komisi)) +(isnull(A.Charge,0)/100)*(SUM (Q' +
        'ty*(Price+Komisi)))-A.Discount+A.Administrasi+(Case When A.FgTax' +
        '= '#39'Y'#39' then (sum(qty*(Price+Komisi))-A.Discount)*10/100 else 0 en' +
        'd),0) as GT ,'
      
        'round(SUM (Qty*(Price+Komisi))+(isnull(A.Charge,0)/100)*(SUM (Qt' +
        'y*(Price+Komisi))) -A.Discount-A.DP+A.Administrasi+(Case When A.' +
        'FgTax= '#39'Y'#39' then ROUND((sum(qty*(Price+Komisi))-A.Discount)*10/10' +
        '0,0) else 0 end),0)  as Total'
      'from ARTrPenjualanhd A INNER JOIN ARTrPenjualanDt B  '
      '        ON A.SaleId=B.SaleId'
      'where B.SaleID=:SaleID'
      'GROUP BY A.FgTax,A.Discount,A.Administrasi,A.Dp,A.Charge')
    Left = 496
    Top = 472
    ...
    object quTotalAdministrasi: TBCDField
      FieldName = 'Administrasi'
      DisplayFormat = '#,0.00'
      Precision = 18
      Size = 0
