inherited fmRptBan: TfmRptBan
  Left = 323
  Top = 312
  Caption = 'Laporan Ban'
  ClientWidth = 1298
  PixelsPerInch = 96
  TextHeight = 13
  inherited paTop: TPanel
    Width = 1298
    inherited laTitle: TLabel
      Width = 147
      Caption = 'Pemakaian Ban'
    end
  end
  inherited paCenter: TPanel
    Width = 1298
    object grbKartu: TGroupBox
      Left = 8
      Top = 6
      Width = 397
      Height = 57
      Caption = '[ Periode ]'
      TabOrder = 0
      object Label2: TLabel
        Left = 24
        Top = 24
        Width = 25
        Height = 13
        Caption = 'Dari :'
      end
      object Label3: TLabel
        Left = 152
        Top = 25
        Width = 41
        Height = 13
        Caption = 'Sampai :'
      end
      object bbRefresh: TBitBtn
        Left = 305
        Top = 15
        Width = 84
        Height = 30
        Hint = 'Refresh'
        Caption = '&Refresh'
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnClick = bbRefreshClick
        Glyph.Data = {
          
          C43337777777777777F334444444444444333777777777777733}
        NumGlyphs = 2
      end
      object dtpSmp: TDateTimePicker
        Left = 200
        Top = 21
        Width = 85
        Height = 21
        CalAlignment = dtaLeft
        Date = 38070.9999884259
        Format = 'dd/MM/yyyy'
        Time = 38070.9999884259
        Color = 16311512
        DateFormat = dfShort
        DateMode = dmComboBox
        Kind = dtkDate
        ParseInput = False
        TabOrder = 1
      end
      object dtpDari: TDateTimePicker
        Left = 56
        Top = 21
        Width = 85
        Height = 21
        CalAlignment = dtaLeft
        Date = 38070
        Format = 'dd/MM/yyyy'
        Time = 38070
        Color = 16311512
        DateFormat = dfShort
        DateMode = dmComboBox
        Kind = dtkDate
        ParseInput = False
        TabOrder = 2
      end
    end
    object bbExp: TBitBtn
      Left = 1145
      Top = 23
      Width = 41
      Height = 25
      Hint = 'Tampilkan semua'
      Anchors = [akTop, akRight]
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      Visible = False
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
    end
    object bbColp: TBitBtn
      Left = 1193
      Top = 23
      Width = 41
      Height = 25
      Hint = 'Sembunyikan semua'
      Anchors = [akTop, akRight]
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      Visible = False
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
    end
    object dgrReport: TdxDBGrid
      Left = 242
      Top = 67
      Width = 1055
      Height = 421
      Bands = <
        item
          Width = 1263
        end>
      DefaultLayout = False
      HeaderPanelRowCount = 1
      KeyField = 'TransID'
      ShowGroupPanel = True
      ShowSummaryFooter = True
      SummaryGroups = <
        item
          DefaultGroup = True
          SummaryItems = <
            item
              SummaryField = 'Jumlah'
              SummaryFormat = '#,0.00'
              SummaryType = cstSum
            end>
          Name = 'dgrReportSummaryGroup2'
        end>
      SummarySeparator = ', '
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      BandColor = clMoneyGreen
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = [fsBold]
      DataSource = dsReport
      Filter.Active = True
      Filter.Criteria = {00000000}
      HeaderColor = clSilver
      HeaderFont.Charset = DEFAULT_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -9
      HeaderFont.Name = 'MS Sans Serif'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      HighlightTextColor = clRed
      LookAndFeel = lfUltraFlat
      OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoEnterThrough, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      ShowBands = True
      ShowRowFooter = True
      object dgrReportColumn1: TdxDBGridColumn
        Caption = 'Tanggal'
        Width = 70
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Tgl'
      end
      object dgrReportColumn2: TdxDBGridColumn
        Caption = 'Nota'
        Width = 74
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TransID'
      end
      object dgrReportColumn3: TdxDBGridColumn
        Caption = 'No Unit'
        Width = 62
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NoUnit'
      end
      object dgrReportColumn16: TdxDBGridColumn
        Caption = 'Group Unit'
        Width = 100
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NmMaster'
      end
      object dgrReportColumn4: TdxDBGridColumn
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'KM'
      end
      object dgrReportColumn5: TdxDBGridColumn
        Visible = False
        Width = 67
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Merk'
      end
      object dgrReportColumn6: TdxDBGridColumn
        Caption = 'Kode Barang'
        Width = 85
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ItemID'
      end
      object dgrReportColumn7: TdxDBGridColumn
        Caption = 'No SERI'
        Width = 68
        BandIndex = 0
        RowIndex = 0
        FieldName = 'serial'
      end
      object dgrReportColumn8: TdxDBGridColumn
        Width = 61
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Jumlah'
        SummaryFooterField = 'Jumlah'
        SummaryFooterFormat = '#,0.00'
        SummaryField = 'Jumlah'
        SummaryFormat = '#,0.00'
        SummaryGroupName = 'dgrReportSummaryGroup2'
      end
      object dgrReportColumn9: TdxDBGridColumn
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Posisi'
      end
      object dgrReportColumn10: TdxDBGridColumn
        Width = 56
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Kondisi'
      end
      object dgrReportColumn11: TdxDBGridColumn
        Width = 62
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Status'
      end
      object dgrReportColumn12: TdxDBGridColumn
        Caption = 'Merk/Keterangan'
        Width = 69
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Note'
      end
      object dgrReportColumn13: TdxDBGridColumn
        Caption = 'Site'
        Width = 77
        BandIndex = 0
        RowIndex = 0
        FieldName = 'WarehouseID'
      end
      object dgrReportColumn14: TdxDBGridColumn
        Width = 81
        BandIndex = 0
        RowIndex = 0
        FieldName = 'UpdDate'
      end
      object dgrReportColumn15: TdxDBGridColumn
        Width = 60
        BandIndex = 0
        RowIndex = 0
        FieldName = 'UpdUser'
      end
    end
    object dbgList: TdxDBGrid
      Left = 1
      Top = 99
      Width = 240
      Height = 214
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'ItemID'
      SummaryGroups = <>
      SummarySeparator = ', '
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = []
      DataSource = dsItem
      Filter.Criteria = {00000000}
      HeaderFont.Charset = ANSI_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -9
      HeaderFont.Name = 'MS Sans Serif'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      LookAndFeel = lfFlat
      OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoIndicator, edgoRowSelect, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      object dbgListSalesID: TdxDBGridMaskColumn
        Caption = 'Part Num'
        Sorted = csUp
        Width = 90
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ItemID'
      end
      object dbgListSalesName: TdxDBGridMaskColumn
        Caption = 'Nama Barang'
        Width = 136
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ItemName'
      end
    end
    object dxDBGrid1: TdxDBGrid
      Left = 1
      Top = 344
      Width = 240
      Height = 144
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'WarehouseID'
      SummaryGroups = <>
      SummarySeparator = ', '
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = []
      DataSource = dsSite
      Filter.Criteria = {00000000}
      HeaderFont.Charset = ANSI_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -9
      HeaderFont.Name = 'MS Sans Serif'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      LookAndFeel = lfFlat
      OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoIndicator, edgoRowSelect, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      object dxDBGridMaskColumn1: TdxDBGridMaskColumn
        Caption = 'Kode Gudang'
        Sorted = csUp
        Width = 80
        BandIndex = 0
        RowIndex = 0
        FieldName = 'WarehouseID'
      end
      object dxDBGridMaskColumn2: TdxDBGridMaskColumn
        Caption = 'Nama Gudang'
        Width = 146
        BandIndex = 0
        RowIndex = 0
        FieldName = 'WarehouseName'
      end
    end
    object Panel1: TPanel
      Left = 8
      Top = 70
      Width = 185
      Height = 26
      Color = 14401101
      TabOrder = 6
      object rbAll: TRadioButton
        Left = 19
        Top = 5
        Width = 53
        Height = 17
        Caption = '&All'
        Checked = True
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Times New Roman'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        TabStop = True
        OnClick = rbAllClick
      end
      object rbSelect: TRadioButton
        Left = 82
        Top = 5
        Width = 61
        Height = 17
        Caption = '&Select'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Times New Roman'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        OnClick = rbAllClick
      end
    end
    object Panel2: TPanel
      Left = 8
      Top = 316
      Width = 185
      Height = 26
      Color = 14401101
      TabOrder = 7
      object RadioButton1: TRadioButton
        Left = 19
        Top = 5
        Width = 53
        Height = 17
        Caption = '&All'
        Checked = True
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Times New Roman'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        TabStop = True
        OnClick = RadioButton1Click
      end
      object RadioButton2: TRadioButton
        Left = 82
        Top = 5
        Width = 61
        Height = 17
        Caption = '&Select'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Times New Roman'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        OnClick = RadioButton1Click
      end
    end
    object Button1: TButton
      Left = 196
      Top = 71
      Width = 42
      Height = 25
      Caption = 'Load'
      TabOrder = 8
      OnClick = Button1Click
    end
    object Button2: TButton
      Left = 196
      Top = 317
      Width = 42
      Height = 25
      Caption = 'Load'
      TabOrder = 9
      OnClick = Button2Click
    end
  end
  inherited paBottom: TPanel
    Width = 1298
    object Label1: TLabel [0]
      Left = 295
      Top = 8
      Width = 95
      Height = 17
      Alignment = taRightJustify
      Caption = 'Jumlah Pasang :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object DBText1: TDBText [1]
      Left = 399
      Top = 8
      Width = 54
      Height = 17
      AutoSize = True
      DataField = 'Pasang'
      DataSource = dsTotal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel [2]
      Left = 963
      Top = 8
      Width = 43
      Height = 17
      Alignment = taRightJustify
      Caption = 'Lepas :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object DBText2: TDBText [3]
      Left = 1015
      Top = 8
      Width = 54
      Height = 17
      AutoSize = True
      DataField = 'Lepas'
      DataSource = dsTotal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    object Label5: TLabel [4]
      Left = 477
      Top = 8
      Width = 41
      Height = 17
      Alignment = taRightJustify
      Caption = 'Spare :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object DBText3: TDBText [5]
      Left = 527
      Top = 8
      Width = 54
      Height = 17
      AutoSize = True
      DataField = 'Stok'
      DataSource = dsTotal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
    end
    object Label6: TLabel [6]
      Left = 725
      Top = 8
      Width = 41
      Height = 17
      Alignment = taRightJustify
      Caption = 'Spare :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object DBText4: TDBText [7]
      Left = 775
      Top = 8
      Width = 54
      Height = 17
      AutoSize = True
      DataField = 'Spare'
      DataSource = dsTotal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    object Label7: TLabel [8]
      Left = 842
      Top = 8
      Width = 44
      Height = 17
      Alignment = taRightJustify
      Caption = 'Rotasi :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object DBText5: TDBText [9]
      Left = 895
      Top = 8
      Width = 54
      Height = 17
      AutoSize = True
      DataField = 'Rotasi'
      DataSource = dsTotal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    inherited bbPreview: TBitBtn
      Left = 10
      Enabled = False
      Visible = False
    end
    inherited bbPrint: TBitBtn
      Left = 10
      Enabled = False
      Visible = False
    end
    inherited bbExit: TBitBtn
      Left = 10
      Enabled = False
      Visible = False
    end
    object bbExcel: TBitBtn
      Left = 1091
      Top = 6
      Width = 115
      Height = 30
      Cursor = crHandPoint
      Anchors = [akRight, akBottom]
      Caption = '&Export Excel'
      TabOrder = 3
      OnClick = bbExcelClick
      Glyph.Data = {
        
        80808080808080800000}
    end
    object bbCancel: TBitBtn
      Left = 1206
      Top = 6
      Width = 92
      Height = 30
      Cursor = crHandPoint
      Anchors = [akRight, akBottom]
      Caption = '&Batal'
      TabOrder = 4
      OnClick = bbCancelClick
      Glyph.Data = {
        
        3393333777333777337333333999993333333333377777333333}
      NumGlyphs = 2
    end
  end
  inherited quAct: TADOQuery
    Top = 8
  end
  inherited dsAct: TDataSource
    Top = 8
  end
  object quReport: TADOQuery
    Connection = dmMain.dbConn
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select CONVERT(VARCHAR(12),B.TransDate,111) as Tgl,A.TransID,A.N' +
        'oUnit,A.KMawal as KM,C.Merk,A.ItemID,A.serial,A.Qty as Jumlah,A.' +
        'Nomorban as Posisi,'
      
        'CASE WHEN A.Statusban='#39'BR'#39' THEN '#39'Baru'#39' ELSE '#39'Bekas'#39' END as Kondi' +
        'si,'
      
        'CASE WHEN A.FgStatus='#39'T'#39' THEN '#39'Lepas'#39' WHEN A.FgStatus='#39'K'#39' THEN '#39 +
        'Pasang'#39'  WHEN A.FgStatus='#39'L'#39' THEN '#39'Stock'#39' ELSE '#39'Scrap'#39' END as St' +
        'atus,A.Alasan as Note,'
      'B.WarehouseID,A.UpdDate,A.UpdUser,E.NmMaster'
      'from INTrAdjustmentDt A '
      'inner join INTrAdjustmentHd B on A.TransID=B.TransID'
      'inner join INMsItem C on A.ItemID=C.ItemID'
      'inner join INMsNoUnit D on A.NoUnit=D.NoUnit'
      'left join AllMaster E on D.groupid=E.KdMaster AND E.FgMaster='#39'U'#39
      'Where B.FlagIN='#39'TB'#39)
    Left = 396
    Top = 8
    object quReportTgl: TStringField
      FieldName = 'Tgl'
      ReadOnly = True
      Size = 12
    end
    object quReportTransID: TStringField
      FieldName = 'TransID'
    end
    object quReportNoUnit: TStringField
      FieldName = 'NoUnit'
      Size = 50
    end
    object quReportKM: TStringField
      FieldName = 'KM'
    end
    object quReportMerk: TStringField
      FieldName = 'Merk'
      Size = 50
    end
    object quReportItemID: TStringField
      FieldName = 'ItemID'
      Size = 100
    end
    object quReportserial: TStringField
      FieldName = 'serial'
      Size = 50
    end
    object quReportJumlah: TBCDField
      FieldName = 'Jumlah'
      Precision = 18
    end
    object quReportPosisi: TStringField
      FieldName = 'Posisi'
    end
    object quReportKondisi: TStringField
      FieldName = 'Kondisi'
      ReadOnly = True
      Size = 5
    end
    object quReportStatus: TStringField
      FieldName = 'Status'
      ReadOnly = True
      Size = 6
    end
    object quReportNote: TStringField
      FieldName = 'Note'
      Size = 150
    end
    object quReportWarehouseID: TStringField
      FieldName = 'WarehouseID'
    end
    object quReportUpdDate: TDateTimeField
      FieldName = 'UpdDate'
    end
    object quReportUpdUser: TStringField
      FieldName = 'UpdUser'
    end
    object quReportNmMaster: TStringField
      FieldName = 'NmMaster'
      Size = 255
    end
  end
  object dsReport: TDataSource
    AutoEdit = False
    DataSet = quReport
    Left = 424
    Top = 8
  end
  object saveDlg: TSaveDialog
    DefaultExt = '*.xls'
    Filter = 'Microsoft Excel File|*.xls'
    Left = 560
    Top = 8
  end
  object dxReport: TdxComponentPrinter
    Version = 0
    Left = 588
    Top = 8
  end
  object quItem: TADOQuery
    Connection = dmMain.dbConn
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select itemid,itemname from inmsitem')
    Left = 644
    Top = 8
    object quItemItemID: TStringField
      FieldName = 'ItemID'
      Size = 50
    end
    object quItemItemName: TStringField
      FieldName = 'ItemName'
      Size = 150
    end
  end
  object dsItem: TDataSource
    AutoEdit = False
    DataSet = quItem
    Left = 672
    Top = 8
  end
  object quSite: TADOQuery
    Connection = dmMain.dbConn
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select warehouseid,warehousename from inmswarehouse')
    Left = 708
    Top = 8
    object quSiteWarehouseID: TStringField
      FieldName = 'WarehouseID'
    end
    object quSiteWarehouseName: TStringField
      FieldName = 'WarehouseName'
      Size = 100
    end
  end
  object dsSite: TDataSource
    AutoEdit = False
    DataSet = quSite
    Left = 736
    Top = 8
  end
  object quTotal: TADOQuery
    Connection = dmMain.dbConn
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select '
      'ISNULL((select SUM(A.Qty) as Jumlah'
      
        'from INTrAdjustmentDt A inner join INTrAdjustmentHd B on A.Trans' +
        'ID=B.TransID'
      'Where B.FlagIN='#39'TB'#39' AND A.FgStatus='#39'T'#39'),0) as Lepas,'
      'ISNULL((select SUM(A.Qty) as Jumlah'
      
        'from INTrAdjustmentDt A inner join INTrAdjustmentHd B on A.Trans' +
        'ID=B.TransID'
      'Where B.FlagIN='#39'TB'#39' AND A.FgStatus='#39'K'#39'),0) as Pasang,'
      'ISNULL((select SUM(A.Qty) as Jumlah'
      
        'from INTrAdjustmentDt A inner join INTrAdjustmentHd B on A.Trans' +
        'ID=B.TransID'
      'Where B.FlagIN='#39'TB'#39' AND A.FgStatus='#39'S'#39'),0) as Spare,'
      'ISNULL((select SUM(A.Qty) as Jumlah'
      
        'from INTrAdjustmentDt A inner join INTrAdjustmentHd B on A.Trans' +
        'ID=B.TransID'
      'Where B.FlagIN='#39'TB'#39' AND A.FgStatus='#39'R'#39'),0) as Rotasi,'
      'ISNULL((select SUM(A.Qty) as Jumlah'
      
        'from INTrAdjustmentDt A inner join INTrAdjustmentHd B on A.Trans' +
        'ID=B.TransID'
      'Where B.FlagIN='#39'TB'#39' AND A.FgStatus='#39'L'#39'),0) as Stok')
    Left = 252
    Top = 8
    object quTotalLepas: TBCDField
      FieldName = 'Lepas'
      ReadOnly = True
      DisplayFormat = '#,0'
      Precision = 32
    end
    object quTotalPasang: TBCDField
      FieldName = 'Pasang'
      ReadOnly = True
      DisplayFormat = '#,0'
      Precision = 32
    end
    object quTotalSpare: TBCDField
      FieldName = 'Spare'
      ReadOnly = True
      DisplayFormat = '#,0'
      Precision = 32
    end
    object quTotalRotasi: TBCDField
      FieldName = 'Rotasi'
      ReadOnly = True
      DisplayFormat = '#,0'
      Precision = 32
    end
    object quTotalStok: TBCDField
      FieldName = 'Stok'
      ReadOnly = True
      DisplayFormat = '#,0'
      Precision = 32
    end
  end
  object dsTotal: TDataSource
    AutoEdit = False
    DataSet = quTotal
    Left = 280
    Top = 8
  end
end
