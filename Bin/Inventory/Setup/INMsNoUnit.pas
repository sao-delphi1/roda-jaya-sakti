...
type
  TfmINMsNoUnit = class(TfmStdLv31)
    ...
    dbgProductLokasi: TdxDBGridButtonColumn;
    dbgProductLLokasi: TdxDBGridColumn;
    quMainSNID: TStringField;
    dbgProductColumn12: TdxDBGridColumn;
    ...
    procedure dbgTypeClassUnitButtonClick(Sender: TObject;
      AbsoluteIndex: Integer);
    procedure dbgProductLokasiButtonClick(Sender: TObject;
      AbsoluteIndex: Integer);
  private
...
uses ConMain, MyUnit, UnitGeneral, Search, StdLv1, StdLv0;
...
begin
  ...
  AND (LowerCase(dmMain.UserId)<>'andy') AND (LowerCase(dmMain.UserId)<>'fredy') AND (LowerCase(dmMain.UserId)<>'naimah') then
...
begin
  ...
  SetReadOnly(dbgProductLLokasi,TRUE);
  SetReadOnly(dbgProductUser,TRUE);
  if (GroupUser<>'admin') and (LowerCase(dmMain.UserId)<>'andy') and (LowerCase(dmMain.UserId)<>'naimah') and (LowerCase(dmMain.UserId)<>'fredy') then
...
begin
  ...
  begin
    Close;Clear;
    Add('SELECT TOP 1 Nounit FROM ARTrKonInvPelDt where nounit='''+quMainNoUnit.ASString+''' ');
    Open;
  end;
  if quAct.RecordCount<>0 then
  begin
...
begin
  ...
  begin
    Close;Clear;
    Add('SELECT TOP 1 ISNULL(A.Lokasi,''-'') as Lokasi FROM MTMutasiUnitHd A INNER JOIN MTMutasiUnitDt B ON A.NotaID=B.NotaID '
       +'WHERE B.NoUnit='''+quMainNoUnit.AsString+''' ORDER BY CONVERT(VARCHAR(8),Transdate,112) DESC');
    Open;
  end;
  quMainLLokasi.AsString := quAct.FieldByName('Lokasi').AsString;
end;
...
procedure TfmINMsNoUnit.dbgProductLokasiButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
   with TfmSearch.Create(Self) do
     try
        Title := 'Gudang';
        SQLString := 'SELECT SiteName as Lokasi, SiteID as Kode FROM INMsSites ORDER BY SiteName';
        if ShowModal = MrOK then
        begin
           if quMain.State = dsBrowse then quMain.Edit;
           quMainLokasi.Value := Res[0];
        end;
     finally
        free;
     end;
