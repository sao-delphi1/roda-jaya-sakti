...
begin
  ...
  if TRIM(quMainLValuta.AsString) = '' then quMainCurrId.AsString := 'IDR';
  ...
  with TfmSearch.Create(Self) do
    try
       ...
       SQLString := 'SELECT SalesID as Kode_Karyawan, SalesName as Nama_Karyawan'
                   +' FROM ARMsSales WHERE FgActive<>0'
                   +' ORDER BY SalesName';
       if ShowModal = MrOK then
       begin
...
procedure TfmCFTrPiutangInternal.dxDBButtonEdit1ButtonClick(
  Sender: TObject; AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
...
procedure TfmCFTrPiutangInternal.quMainCalcFields(DataSet: TDataSet);
begin
  inherited;
  with quAct,SQL do
  begin
  ...
  quMainLValuta.AsString := quAct.FieldByName('CurrName').AsString;
end;
