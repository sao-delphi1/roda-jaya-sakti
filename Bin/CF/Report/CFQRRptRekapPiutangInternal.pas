...
begin
  ...
  Begin
    Close;Clear;
    add(' SELECT A.Actor, CONVERT(VARCHAR(10),A.Transdate,103) as Tanggal, A.VoucherID as IP, ISNULL(SUM(B.Amount),0) as Kredit,A.CurrID,'
       +' (SELECT ISNULL(SUM(L.ValuePayment),0) FROM CFTrPiutangINternalDt L'
       +' INNER JOIN CFTrPiutangInternalHd H ON L.PiutangID=H.PiutangID WHERE L.VoucherID=A.VoucherID AND H.SalesID=A.Actor AND H.CurrID=A.CurrID)+ '
       ...
       +' AND A.Actor='''+qu001.FieldByName('SalesID').AsString+''' AND A.CurrID ='''+qu002.FieldByName('CurrID').AsString+''' ');
    Add(' AND CONVERT(varchar(8),A.Transdate,112) <= '''+FormatDateTime('yyyyMMdd',tglDari)+''''
       +' GROUP BY A.Actor,A.Transdate,A.VoucherID,A.CurrID ORDER BY Convert(varchar(20),A.Transdate,112)');
    Open;
  End;
end;
