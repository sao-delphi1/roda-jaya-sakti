inherited fmAPRptLapMR: TfmAPRptLapMR
  Left = 193
  Top = 124
  Caption = 'Laporan MR'
  ClientHeight = 575
  ...
  object quReport: TADOQuery
    ...
    SQL.Strings = (
      ...
      'C.ItemName as Nama_Barang,C.typeunit,A.NoUnit,A.Jumlah,A.UOMID,'
        ...
        'ISNULL((SELECT SUM(X.Jumlah) FROM APTrPurchaseRequestDT X WHERE ' +
        'X.MRID=A.PRID AND X.ItemID=A.ItemID And X.FgNum=A.NUmAll),0) as ' +
        'JumPR,'
      
        'ISNULL((SELECT SUM(X.Qty) FROM INTrAdjustmentDt X Where X.ItemID' +
        '=A.ItemID AND X.MRID=A.PRID AND X.FgNum=A.NumAll),0) as JumPakai' +
        ','
      'D.SiteName,B.DivisiID,'#39'-'#39' as keterangan,A.upduser,A.upddate'
    ...
    object quReportJumPR: TBCDField
      FieldName = 'JumPR'
      ReadOnly = True
      DisplayFormat = '#,0'
      Precision = 32
    end
    object quReportJumPakai: TBCDField
      FieldName = 'JumPakai'
      ReadOnly = True
      DisplayFormat = '#,0'
      Precision = 32
