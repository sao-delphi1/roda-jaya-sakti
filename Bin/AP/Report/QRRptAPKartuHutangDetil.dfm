inherited fmAPQRRptKartuHutangDetil: TfmAPQRRptKartuHutangDetil
  ...
  object qu004: TADOQuery
    ...
    SQL.Strings = (
      ...
      'CASE WHEN B.BankID IS NULL THEN '#39'KAS'#39' ELSE '
      
        '(SELECT X.BankName FROM CFMsBank X WHERE X.BankID=B.BankID) END ' +
        'as Status,'
      ...
      'ISNULL(ValuePayment,0) as Amount FROM APTrPaymentDt A '
        ...
        'INNER JOIN APTrKonsinyasiInvHD C ON A.PurchaseId=C.KonsinyasiInv' +
        'ID '
      'INNER JOIN APTrPayMentHd B ON A.PayMentId=B.PayMentId ')
    Left = 404
    Top = 100
