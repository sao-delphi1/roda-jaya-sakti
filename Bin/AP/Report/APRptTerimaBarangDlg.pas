...
begin
  ...
  with TfmAPQRRptTerimaBarang.Create(Self) do
     try
       ...
       begin
           Close;Clear;
           add(' select A.KonsinyasiId,A.BPBID as NoBPB,A.POID,Convert(Varchar(10),Transdate,103) as Tgl,'
              +' A.SuppId,B.SuppName as CustName,A.WareHouseId,C.WareHouseName,'
              +' A.Note,A.POID '
           ...
           add(' ORDER by Convert(Varchar(10),A.Transdate,112),A.NoBPB');
           Open;
          if IsEmpty then
          begin
             MsgInfo('No Data !');
       ...
       begin
          Close;Clear;
          add(' SELECT A.ItemId,B.ItemName,B.UOMID2,A.QTY,A.TypeUnit,A.NoUnit  '
