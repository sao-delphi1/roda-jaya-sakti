...
begin
  FFieldOrder := 'CONVERT(VARCHAR(8),TransDate1,112), KonsinyasiInvID';
  FFieldTgl   := 'TransDate';
...
begin
  ...
  Begin
    MsgInfo('Tanggal Jurnal tidak boleh kosong');
...
begin
  ...
  if StrToInt(FormatDateTime('yyyyMMdd',quMainTransDate1.AsDateTime))>=20220401 then
...
begin
  ...
  with TfmSearch.Create(Self) do
    try
       ...
       SQLString := ' SELECT A.KonsinyasiInvID as Invoice,Convert(Varchar(10),A.Transdate1,103) as Tgl,'
                   ...
                   +' GROUP BY A.KonsinyasiInvID,A.Transdate1,A.Transdate,B.SuppName,A.CurrID,A.Note,A.POID '
                   +' ORDER BY A.KonsinyasiInvID';
...
{  if quMainFgOto.AsString = 'T' then
  ...
  try
    ...
    Begin
      Close;Clear;
      add('select A.KonsinyasiInvID,CONVERT(VARCHAR(10),A.Transdate1,103) as Tanggal,ISNULL(A.SiteID+''-'','''')+A.Note as note,'''' as Lokasi, '
...
procedure TfmAPKonsinyasiInvIDR.dxDBButtonEdit17ButtonClick(
  Sender: TObject; AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
...
procedure TfmAPKonsinyasiInvIDR.dxDBButtonEdit18ButtonClick(
  Sender: TObject; AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
...
procedure TfmAPKonsinyasiInvIDR.dxDBButtonEdit19ButtonClick(
  Sender: TObject; AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
