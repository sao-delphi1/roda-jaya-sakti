...
Begin
 ...
 Begin
   Close;Clear;
   add(' Update APTrPurchaseHd SET TTLPb='+CurrToStr(quTotalTotal.AsCurrency)+',Stpb='+CurrToStr(quTotalSubTotal.AsCurrency)+','
      +' PPN='+CurrToStr(quTotalPajak.AsCurrency)+',PPH42='+CurrToStr(quTotalPPH42.AsCurrency)+','
      +' DiscAmount='+CurrToStr(quTotalDisc.AsCurrency)+',SubTotalU='+CurrToStr(quTotalSubTotalU.AsCurrency)+', '
      +' PPH21='+CurrToStr(quTotalPPH21.AsCurrency)+',PPH23='+CurrToStr(quTotalPPH23.AsCurrency)+', '
      +' PPH15='+CurrToStr(quTotalPPH15.AsCurrency)+',PPH26='+CurrToStr(quTotalPPH26.AsCurrency)+' '
...
begin
  ...
  quMainBiayaMaterial.AsCurrency := 0;
...
begin
  ...
  {if quMainFgOto.AsString = 'T' then
  begin
    ShowMessage('FPP belum di otorisasi, harap otorisasi sebelum cetak');
    abort;
  end else           }
  ...
  try
    ...
    qr11.Caption := CurrToStr(quMainBiayaMaterial.AsCurrency);
    ...
    if (quMainNilaiTax.AsString = '1.1') or (quMainNilaiTax.AsString = '1') then
    QRLabel32.Caption := 'VAT (11%)'
    else
    QRLabel32.Caption := 'VAT ('+quMainNilaiTax.AsString+ '%)';
...
procedure TfmAPPurchaseOrder.dxDBButtonEdit11ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
...
procedure TfmAPPurchaseOrder.dxDBButtonEdit17ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
...
procedure TfmAPPurchaseOrder.dxDBButtonEdit19ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
...
procedure TfmAPPurchaseOrder.dxDBButtonEdit18ButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
  with TfmSearch.Create(Self) do
